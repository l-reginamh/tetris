document.addEventListener('DOMContentLoaded', () => {
    window.addEventListener('resize', function(event) {
        if (window.innerWidth < 576) {
            if (document.getElementById('dialog-default').hasAttribute('open')) {
                // do nothing
            }
            else {
    ...
    const gameOverScene = document.querySelector('.game-over');
    const width = 10
    let nextRandom = 0
    let timerId
    let score = 0
    ...
    const theTetrominoes = [lTetromino, zTetromino, rzTetromino, tTetromino, oTetromino, iTetromino]
    ...
    function gameOver() {
        if (current.some(index => squares[currentPosition + index].classList.contains('taken'))) {
            ...
            gameOverScene.classList.add('show')
            gameOverScene.classList.remove('hide');
            if (startBtn.innerText.includes('Pause')) {
